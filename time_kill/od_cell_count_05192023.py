...
ax.errorbar(od_measured,cell_count_avg,yerr=cell_count_err,xerr=od_measured_err,fmt='o',capsize=5)
ax.set_yscale('log')
ax.set_xscale('log')
...
ax.plot(od_avg,cell_count_extrap,'*')
...
xfit = np.linspace(np.min(od_measured),np.max(od_measured),100)
